...
export function getFlatNode(root: FlatNode | undefined, offset: number, includeNodeBoundary: boolean): FlatNode | undefined {
	...
	let currentNode = root.firstChild;
	let foundNode: FlatNode | undefined = undefined;

	while (currentNode) {
		const nodeStart: number = currentNode.start;
		const nodeEnd: number = currentNode.end;
...
export function getHtmlFlatNode(documentText: string, root: FlatNode | undefined, offset: number, includeNodeBoundary: boolean): HtmlFlatNode | undefined {
	...
	// 	selectionStartOffset--;
	// }
	return getHtmlFlatNodeInternal(documentText, root, offset, includeNodeBoundary);
}

function getHtmlFlatNodeInternal(documentText: string, root: FlatNode | undefined, offset: number, includeNodeBoundary: boolean = false): HtmlFlatNode | undefined {
	...
	if (isTemplateScript
		...
		const scriptBodyNode = getHtmlFlatNodeInternal(scriptBodyText, innerRoot, offset, includeNodeBoundary);
		if (scriptBodyNode) {
			scriptBodyNode.parent = currentNode;
			currentNode.children.push(scriptBodyNode);
			if (currentNode.children.length >= 2) {
				const penultimateChild = currentNode.children[currentNode.children.length - 2];
				penultimateChild.nextSibling = currentNode;
				currentNode.previousSibling = penultimateChild;
			}
